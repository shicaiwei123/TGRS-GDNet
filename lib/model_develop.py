...
def train_base_multi_PE(model, cost, optimizer, train_loader, test_loader, args):
    print(args)
    ...
    while epoch < epoch_num:
        for batch_idx, batch_sample in enumerate(
            ...
            x1, x2, x3, hsi_mu, hsi_std, lidar_mul, lidar_std = model(img_hsi, img_lidar)

            regurize_a = regurize(hsi_mu, hsi_std)
            regurize_a = regurize_a.cuda()

            regurize_v = regurize(lidar_mul, lidar_std)
            regurize_v = regurize_v.cuda()

            cls_loss = cost(x1, target)

            rec_loss_1 = mse_func(x2, x1)
            rec_loss_2 = mse_func(x3, x1)

            out_a_s = F.softmax(x2, dim=1)
            H_a = torch.log(out_a_s + 1e-16) * out_a_s
            H_a = -torch.sum(H_a, dim=1, keepdim=True)

            out_v_s = F.softmax(x3, dim=1)
            H_v = torch.log(out_v_s + 1e-16) * out_v_s
            H_v = -torch.sum(H_v, dim=1, keepdim=True)

            # weight = H_a / (H_v + 1e-16)

            weight = torch.cat([H_a, H_v], dim=1)
            # print(weight)
            ...
            unimodal_loss = criterion(x2, target) * weight[:, 1] + criterion(x3, target) * weight[:, 0]
            unimodal_loss = unimodal_loss.mean()

            rec_loss = rec_loss_1 + rec_loss_2

            loss_cls = cls_loss + rec_loss + unimodal_loss
            rec_loss_sum += rec_loss

            loss = loss_cls + (regurize_a + regurize_v) * 1e-4
            # print(regurize_a + regurize_v)
...
def train_base_multi_PE_Formal(model, cost, optimizer, train_loader, test_loader, args):
    print(args)
    ...
    while epoch < epoch_num:
        for batch_idx, batch_sample in enumerate(
            ...
            x1, x2, x3, hsi_mu, hsi_std, lidar_mul, lidar_std, x2_h, x3_l = model(img_hsi, img_lidar)
            #
            # if (x2 - x2_h).sum()!=0 or (x3 - x3_l).sum()!=0:
            #     raise ValueError
            ...
            out_a_s = F.softmax(x2_h, dim=1)
            H_a = torch.log(out_a_s + 1e-16) * out_a_s
            H_a = -torch.sum(H_a, dim=1, keepdim=True)

            out_v_s = F.softmax(x3_l, dim=1)
            H_v = torch.log(out_v_s + 1e-16) * out_v_s
            H_v = -torch.sum(H_v, dim=1, keepdim=True)
            ...
            # unimodal_loss = criterion(x2_h, target) * weight[:, 1] + criterion(x3_l, target) * weight[:, 0] + F.mse_loss(x2_h, x1) + F.mse_loss(x3_l, x1)
            # unimodal_loss = unimodal_loss.mean()

            # unimodal_loss = (criterion(x2_h, target) * weight[:, 1] + criterion(x3_l, target) * weight[:, 0] ).mean() +\
            #                 (F.mse_loss(x2_h, x1,reduction='none').sum(dim=1)* weight[:, 1] + F.mse_loss(x3_l, x1,reduction='none').sum(dim=1)* weight[:, 0]).mean()   # 不太行

            unimodal_loss = criterion(x2_h, target).mean() + criterion(x3_l, target).mean() + F.mse_loss(x2_h, x1) + F.mse_loss(x3_l,
                                                                                                                  x1)
            loss_cls = cls_loss + unimodal_loss * args.beta
            rec_loss_sum += unimodal_loss.item()
        ...
        if epoch > 300:
            ...
            log_list.append(accuracy_best)
            print(
                "Epoch {}, cls_loss={:.5f},unimodal_loss={:.5f}, accuracy_test={:.5f},  accuracy_best={:.5f}".format(
                    epoch,
                    train_loss / len(
                        train_loader),
                    rec_loss_sum / len(
